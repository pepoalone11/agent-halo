...
When a deadline has passed, the transition is applied once: completed Focus increments the focus count and prepares Short/Long break; a completed break prepares Focus. Completion remains visible briefly in the notch. A naturally completed Focus may also summon the optional event-only Completion Pet described in `docs/pet.md`; no other Pomodoro transition summons it. When explicitly enabled and selected from that Pet, a local Squat or Overhead Reach Movement Break described in `docs/movement-break.md` may run before the main renderer starts the prepared break.
